...
export default function Header() {
  ...
  const { currentUser, cart, authStatus, verifyingToken, logoutUser, getCart, editCart, setLoading } = userCtx;
  ...
  useEffect(() => {
    ...
    const updatedPrices = currentPizza.prices.map((firstElement) => {
      let comparisonCart = cart.filter((secondElement) => firstElement.id === secondElement.priceID);
      const [cartQuantity] = comparisonCart;
      return { ...firstElement, quantity: cartQuantity ? cartQuantity.quantity : 0 };
    });
  ...
  useEffect(() => {
    const getTotalProducts = () => {
      return cart.reduce((acc, cv) => acc + cv.quantity, 0);
    };
  ...
  useEffect(() => {
    ...
    if (success) {
      setShowOn({ show: true, msg: "Pago realizado con éxito", cta: "Ve tu recibo", ctaURL: "/perfil" });
      return editCart([]);
    }

    if (canceled) {
      return setShowOn({ show: true, msg: "Hubo problemas con el pago, revisa nuevamente.", cta: "Ir al carrito", ctaURL: "/carrito" });
    }
  }, []);
      ...
      <nav className="flex justify-between mx-8 py-4">
        <ul className="flex items-center">
          ...
          <li className="hidden ml-10 text-gray-900 md:block">
            <Link to="/pizzas" className="font-medium hover:scale-105 transition-transform duration-200">
              Menú
            </Link>
          </li>
        </ul>
        <section className="flex items-center justify-end">
          {authStatus ? (
            ...
            <>
              <Link to="/registro" className="btn-nav bg-gray-800 text-white rounded-lg px-4 py-2 mr-4">Crear cuenta</Link>
              <Link to="/iniciar-sesion" className="btn-nav bg-gray-800 text-white rounded-lg px-4 py-2">Iniciar sesión</Link>
            </>
          )}
        </section>
      </nav>
          ...
          <Transition show={show} as={Fragment}>
            <div className="w-full bg-white shadow-lg border pointer-events-auto p-4 lg:w-1/4">
              <section className="flex">
                <figure>
                  <img className="pt-1 h-10 w-10" src={PizzaLogo} alt="Admin Logo" />
                </figure>
                <article className="ml-8 flex-1">
                  ...
                  <Link to={ctaURL} onClick={() => setShowOff()}>
                    <button type="button" className="mt-4 py-2 px-4 text-sm font-medium text-white bg-brand-purple">
                      {cta}
                    </button>
                  </Link>
                </article>
                <article className="ml-4">
                  <button className="text-black" onClick={() => setShowOff()}>
